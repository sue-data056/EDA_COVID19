...
> ## 4. 분석 지표: 5년전의 우리나를 위협했던 메르스 바이러스 상황을 분석 지표로 삼아 기사 및 매거진 참고
...
> [유동 인구와 매출 증감량 관계] / [주민등록상 인구와 매출 증감량 관계]
...
# 한계점 
1) 너무 많은 요소가 영향을 미치기 때문에 결과가 유기적이었습니다. 즉, 완벽한 선형성을 보이는 요소를 찾는 것이 불가능하였습니다. 그러나보니, 예외 상황을 설명하는 변수를 찾기 위해서 계속 다양한 데이터들을 파고들었고, 이로 인해 설명되지 않는 상황들이 생겨나게 되었다는 것이 가장 큰 한계점이었습니다.
